...
from trading.domaintypes import DateSum
...
def printHprs(hprs: list[DateSum]):
	for item in hprs:
		print(f"{item.DateTime.strftime(trading.settings.displayDateLayout)} {hprDisplay(item.Sum):.1f}%")
...
def printDrawdownInfo(data: DrawdownInfo):
	...
	print(f"Дата максимума эквити {data.HighEquityDate.strftime(trading.settings.displayDateLayout)}")
